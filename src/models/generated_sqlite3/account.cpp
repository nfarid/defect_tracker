...
const uint64_t& Account::getValueOfId() const noexcept
{
...
const std::shared_ptr<uint64_t>& Account::getId() const noexcept
{
    return id_;
}

void Account::setId(const uint64_t& pId) noexcept
{
...
{
    const static std::string sql = "select * from project where manager_id = ?";
    *clientPtr << sql
               << *id_
...
void Account::getTickets(const DbClientPtr& clientPtr,
        ...
        const ExceptionCallback& ecb) const
{
    const static std::string sql = "select * from ticket where reporter_id = ?";
    *clientPtr << sql
               << *id_
        >>[rcb = std::move(rcb)](const Result& r){
        ...
        const ExceptionCallback& ecb) const
{
    const static std::string sql =
            "select * from project,staff where staff.staff_id = ? and staff.project_id = project.id";
    *clientPtr << sql
               << *id_
        >>[rcb = std::move(rcb)](const Result& r){
        ...
        for(auto const& row : r)
        {
            ret.emplace_back(std::pair<Project, Staff>(
                            Project(row), Staff(row, Project::getColumnNumber() ) ) );
        }
        rcb(ret);
        }
        >> ecb;
