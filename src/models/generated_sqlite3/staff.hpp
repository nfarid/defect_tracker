...
public:
    ...
    const uint64_t& getValueOfStaffId() const noexcept;
    ///Return a shared_ptr object pointing to the column const value, or an empty shared_ptr object if the column is null
    const std::shared_ptr<uint64_t>& getStaffId() const noexcept;
    ///Set the value of the column staff_id
    void setStaffId(const uint64_t& pStaffId) noexcept;
...
private:
    ...
    std::shared_ptr<uint64_t> staffId_;
    struct MetaData{
