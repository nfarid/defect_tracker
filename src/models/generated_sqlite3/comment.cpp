...
const std::vector<typename Comment::MetaData> Comment::metaData_={
    ...
    {"poster_id", "uint64_t", "integer", 8, 0, 0, 1}
};
...
Comment::Comment(const Row& r, const ssize_t indexOffset) noexcept
{
    if(indexOffset < 0) {
        ...
        if(!r["poster_id"].isNull() )
            posterId_=std::make_shared<uint64_t>(r["poster_id"].as<uint64_t>() );
    } else {
        ...
        index = offset + 4;
        if(!r[index].isNull() )
            posterId_=std::make_shared<uint64_t>(r[index].as<uint64_t>() );
    }
}
...
Comment::Comment(const Json::Value& pJson, const std::vector<std::string>& pMasqueradingVector) noexcept(false)
{
    ...
    if(!pMasqueradingVector[4].empty() && pJson.isMember(pMasqueradingVector[4]) ) {
        ...
        if(!pJson[pMasqueradingVector[4]].isNull() )
            posterId_=std::make_shared<uint64_t>( (uint64_t)pJson[pMasqueradingVector[4]].asUInt64() );
    }
}
...
Comment::Comment(const Json::Value& pJson) noexcept(false)
{
    ...
    if(pJson.isMember("poster_id") ) {
        ...
        if(!pJson["poster_id"].isNull() )
            posterId_=std::make_shared<uint64_t>( (uint64_t)pJson["poster_id"].asUInt64() );
    }
}
...
const uint64_t& Comment::getValueOfId() const noexcept
{
...
const std::shared_ptr<uint64_t>& Comment::getId() const noexcept
{
    return id_;
}

void Comment::setId(const uint64_t& pId) noexcept
{
    id_ = std::make_shared<uint64_t>(pId);
    dirtyFlag_[0] = true;
}

void Comment::setIdToNull() noexcept
{
    id_.reset();
    dirtyFlag_[0] = true;
}
...
const uint64_t& Comment::getValueOfTicketId() const noexcept
{
    const static uint64_t defaultValue = uint64_t();
    if(ticketId_)
        return *ticketId_;
...
const std::shared_ptr<uint64_t>& Comment::getTicketId() const noexcept
{
    return ticketId_;
}

void Comment::setTicketId(const uint64_t& pTicketId) noexcept
{
    ticketId_ = std::make_shared<uint64_t>(pTicketId);
    dirtyFlag_[3] = true;
}

const uint64_t& Comment::getValueOfPosterId() const noexcept
{
    const static uint64_t defaultValue = uint64_t();
    if(posterId_)
        return *posterId_;
...
const std::shared_ptr<uint64_t>& Comment::getPosterId() const noexcept
{
    return posterId_;
}

void Comment::setPosterId(const uint64_t& pPosterId) noexcept
{
    posterId_ = std::make_shared<uint64_t>(pPosterId);
    dirtyFlag_[4] = true;
}
...
Json::Value Comment::toJson() const
{
    ...
    if(getPosterId() )
        ret["poster_id"]=(Json::UInt64)getValueOfPosterId();
    else
    ...
    const std::vector<std::string>& pMasqueradingVector) const
{
    ...
    if(pMasqueradingVector.size() == 5) {
        ...
        if(!pMasqueradingVector[4].empty() ) {
            if(getPosterId() )
                ret[pMasqueradingVector[4]]=(Json::UInt64)getValueOfPosterId();
            else
...
{
    const static std::string sql = "select * from ticket where id = ?";
    *clientPtr << sql
               << *posterId_
