...
public:
    ...
    const uint64_t& getValueOfManagerId() const noexcept;
    ///Return a shared_ptr object pointing to the column const value, or an empty shared_ptr object if the column is null
    const std::shared_ptr<uint64_t>& getManagerId() const noexcept;
    ///Set the value of the column manager_id
    void setManagerId(const uint64_t& pManagerId) noexcept;
...
private:
    ...
    std::shared_ptr<uint64_t> managerId_;
    struct MetaData{
