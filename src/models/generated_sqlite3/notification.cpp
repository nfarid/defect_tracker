...
const std::vector<typename Notification::MetaData> Notification::metaData_={
    ...
    {"ticket_id", "uint64_t", "integer", 8, 0, 0, 1}
};
...
Notification::Notification(const Row& r, const ssize_t indexOffset) noexcept
{
    if(indexOffset < 0) {
        ...
        if(!r["ticket_id"].isNull() )
            ticketId_=std::make_shared<uint64_t>(r["ticket_id"].as<uint64_t>() );
    } else{
        ...
        index = offset + 3;
        if(!r[index].isNull() )
            ticketId_=std::make_shared<uint64_t>(r[index].as<uint64_t>() );
    }
}
        ...
        std::vector<std::string>& pMasqueradingVector) noexcept(false)
{
    ...
    if(!pMasqueradingVector[3].empty() && pJson.isMember(pMasqueradingVector[3]) ) {
        ...
        if(!pJson[pMasqueradingVector[3]].isNull() )
            ticketId_=std::make_shared<uint64_t>( (uint64_t)pJson[pMasqueradingVector[3]].asUInt64() );
    }
}
...
Notification::Notification(const Json::Value& pJson) noexcept(false)
{
    ...
    if(pJson.isMember("ticket_id") ) {
        ...
        if(!pJson["ticket_id"].isNull() )
            ticketId_=std::make_shared<uint64_t>( (uint64_t)pJson["ticket_id"].asUInt64() );
    }
}
...
const uint64_t& Notification::getValueOfId() const noexcept
{
...
const std::shared_ptr<uint64_t>& Notification::getId() const noexcept
{
    return id_;
}

void Notification::setId(const uint64_t& pId) noexcept
{
    id_ = std::make_shared<uint64_t>(pId);
    dirtyFlag_[0] = true;
}

void Notification::setIdToNull() noexcept
{
    id_.reset();
    dirtyFlag_[0] = true;
}
...
const uint64_t& Notification::getValueOfUserId() const noexcept
{
    const static uint64_t defaultValue = uint64_t();
    if(userId_)
        return *userId_;
...
const std::shared_ptr<uint64_t>& Notification::getUserId() const noexcept
{
    return userId_;
}

void Notification::setUserId(const uint64_t& pUserId) noexcept
{
    userId_ = std::make_shared<uint64_t>(pUserId);
    dirtyFlag_[2] = true;
}

const uint64_t& Notification::getValueOfTicketId() const noexcept
{
    const static uint64_t defaultValue = uint64_t();
    if(ticketId_)
        return *ticketId_;
...
const std::shared_ptr<uint64_t>& Notification::getTicketId() const noexcept
{
    return ticketId_;
}

void Notification::setTicketId(const uint64_t& pTicketId) noexcept
{
    ticketId_ = std::make_shared<uint64_t>(pTicketId);
    dirtyFlag_[3] = true;
}
...
Json::Value Notification::toJson() const
{
    ...
    if(getTicketId() )
        ret["ticket_id"]=(Json::UInt64)getValueOfTicketId();
    else
    ...
    const std::vector<std::string>& pMasqueradingVector) const
{
    ...
    if(pMasqueradingVector.size() == 4) {
        ...
        if(!pMasqueradingVector[3].empty() ) {
            if(getTicketId() )
                ret[pMasqueradingVector[3]]=(Json::UInt64)getValueOfTicketId();
            else
